% ==============================================================================
% SLIDES - SEMANA 03: AUTOMAÇÃO DE BUILDS E INTEGRAÇÃO CONTÍNUA (CI) COM GITHUB ACTIONS
% ==============================================================================

% 1. CAPA
\senaicover
  {Automação de Builds e Integração Contínua (CI) com GitHub Actions}
  {Construção automatizada e validação contínua de código a cada alteração.}
...
% 3. CONTEÚDO TEÓRICO
\begin{senaiframe}{Teoria • Módulo 1}{Automação de Builds e Integração Contínua (CI) com GitHub Actions}
  \begin{columns}[t]
    \begin{column}{0.48\textwidth}
      \begin{tcolorbox}[senaibluecard, title={Fundamentos Teóricos}]
        \begin{itemize}
          \item Workflows em YAML, triggers (push, pull_request), jobs, steps, runners do GitHub e caching de dependências Python.
        \end{itemize}
      \end{tcolorbox}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{tcolorbox}[senaigreencard, title={Prática \& Laboratório}]
        \begin{itemize}
          \item Criação da primeira esteira de CI no GitHub Actions para rodar linters (flake8/black) no projeto FactoryHub.
        \end{itemize}
      \end{tcolorbox}
    \end{column}
  \end{columns}
